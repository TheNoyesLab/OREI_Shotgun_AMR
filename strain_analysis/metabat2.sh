...
do
	...
	bwa mem -t 50 $assemblies/spades_${file}_output/scaffolds.fasta $reads/$file.R1.fastq.gz $reads/$file.R2.fastq.gz > $work/$file.sam
	
	#Make BAM from SAM
	samtools view -bS $work/$file.sam > $work/$file.bam
	...
	metabat2 -i $assemblies/spades_${file}_output/scaffolds.fasta -a $work/depth.txt -o $binning/metabat2/${file}_bins/${file}_bin -t 50 -m 1500
...
do
	...
	#Make SAM of dehosted reads vs Scaffold
	bwa mem -t 50 $assemblies/spades_${file}_output/scaffolds.fasta $coReads/$file.R1.fastq.gz $coReads/$file.R2.fastq.gz > $work/$file.sam
